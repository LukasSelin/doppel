...
be stated as a rate instead of a number. A fixed threshold is loose on a corpus of 81 functions and
strict on one of 8000; "admit 1% of random pairs" means the same thing on both. Measured at rate
0.01 on the public ladder under the WL metric: the calibrated code-shape threshold is **0.33 on
prometheus, 0.35 on moby and hugo, 0.41 on gin, 0.44 on cobra, 0.45 on chi**, and struct-min lands
between 0.30 and 0.44. **conc declines** — 351 eligible shape null pairs against the 1 000 the
calibration requires — which is itself the small-corpus signal the decline rule exists to give.
The median of those six is what the shipped `--threshold` default is set from; see *Where the
shipped floors come from*.
