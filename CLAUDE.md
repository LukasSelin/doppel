...
`dashboard` imports **nothing from this module at all** — its payload is plain data and its
renderer is `html/template` plus `embed` — which is what keeps the page's data contract from
quietly acquiring pipeline types; `cmd` bridges a finished run into it, exactly as it does for
`reporter.Overview`.
...
- **The partition is a control, not a rebuild.** `powerDiagram` does not care what a site is, so the
  ...
  to one region (the arena's dominant survivor, else its first tag) where the tag set is genuinely
  multi-valued — a real simplification, and the reason package is the default.
